...
func (service UserService) Register(ctx context.Context) serializer.Response {
	...
	user := &model.User{
		...
		Status:   model.Active,
		Money:    util2.Encrypt.AesEncoding("0"), // 初始金额
	}
	//加密密码
...
func (service *SendEmailService) Send(ctx context.Context, id uint) serializer.Response {
	...
	token, err := util2.GenerateEmailToken(id, service.OperationType, service.Email, service.Password)
	if err != nil {
		logging.Info(err)
		code = e.ErrorAuthToken
		return serializer.Response{
